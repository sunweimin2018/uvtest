...
for ring in rings:
    ...
    for i in range(n):
        ...
        t_mid = (t0 + t1) / 2
        r_mid = (r0 + r1) / 2
        x = r_mid * np.cos(t_mid)
        y = r_mid * np.sin(t_mid)

        # 切线方向 = 径向 + 90°
        rot = np.degrees(t_mid) + 90
        if 90 < t_mid % (2 * np.pi) < 270:
            rot += 180  # 防止文字倒置
...
# 中心圆
c = plt.Circle((0, 0), 0.65, color="#fdf8f0", ec="#d0c0b0", linewidth=1.5, zorder=10)
ax.add_patch(c)
ax.text(0, 0, "总览", ha="center", va="center", fontsize=18, fontweight="bold", color="#4a3020")
